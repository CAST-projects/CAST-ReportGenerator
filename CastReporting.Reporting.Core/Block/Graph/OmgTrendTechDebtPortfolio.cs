...
{
    [Block("PF_TREND_OMG_TECH_DEBT")]
    public class OmgTrendTechDebtPortfolio : GraphBlock<ImagingData>
    {
                ...
                for (int i = 0; i < dtDates.Rows.Count; i++)
                {
                    ...
                    if (_allSnapshots.Length > 0)
                    {
                        foreach (Snapshot snapshot in _allSnapshots.OrderBy(_ => _.Annotation.Date.DateSnapShot))
                        {
                            ...
                            if (omgTechDebt == null) continue;
                            _removedTechnicalDebt += omgTechDebt.Removed;
                            _addedTechnicalDebt += omgTechDebt.Added;
                            _totalTechnicalDebt += omgTechDebt.Total;
                        }
                    }
